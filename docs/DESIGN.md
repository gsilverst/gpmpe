...
Business data version control is administered through the application, not by regular campaign users. The long-term model is business-oriented: each business profile may be backed by its own Git repository and branch/ref, rather than requiring one deployment-wide repository containing multiple businesses. Primary Admin/Admin users should be able to manage administrator-only business profile fields for repository name/URL, branch or ref, Git author identity, push policy, and credential reference through an administrator page alongside other administrative functions such as adding users, assigning roles, and managing access. Administrators must also configure a runtime Git credential, such as a fine-grained Git token, GitHub App credential, deploy key, or equivalent provider-specific secret, before automated pull/push is enabled for private business data repositories. Raw credentials must remain write-only from the browser perspective and should be stored in the configured secret store rather than in normal application tables.

Local deployments still use one configured `DATA_DIR` from `.config`, and that runtime directory still contains one subdirectory per business. The source-control distinction is that each business directory can be imported from, saved to, and synchronized with a separate business-specific Git repository. This keeps the local/runtime filesystem convenient while making Git ownership align with business ownership.
...
Color editing should not require users to memorize hex values. The application should provide a shared named-color catalog with canonical user-facing names, color families, hex values, and optional aliases. The chat and GUI should use this catalog consistently for brand colors, component colors, item colors, and renderer style parameters.

The promotion editing interface should display common colors as swatches with names, and the chat interface should support discovery queries such as "show me all available shades of green" or "what purple colors can I use". The response should present a palette or list of actual colors, including visible swatches and the names users can reuse in later commands. This lets a user choose between names such as `emerald` and `deep forest` by seeing the colors, then issue commands like "change the background color to deep forest" without needing to remember `#50C878` or another hex value.

Color families should be ordered by shade so users can make relative adjustments after choosing a color. For example, if a background is set to `deep forest`, the user should be able to say "make the background a lighter shade of green" or "make it two shades lighter". The command resolver should identify the current named color or nearest catalog color, move the requested number of steps lighter or darker within the same family, clamp or clarify at palette boundaries, and confirm the resulting color by name.

Internally, named colors should normalize to canonical stored values while preserving enough user-facing metadata to produce helpful confirmations, validations, and documentation. Existing explicit hex commands should remain supported for advanced users.
...
Git settings currently represent global application/service credentials. They are not tied to the signed-in user's personal Git account. The application should provide an administrator-only interface for managing global Git settings in both local and AWS deployments. That admin interface must collect or rotate the runtime Git credential, such as a fine-grained token, GitHub App credential, deploy key, or equivalent secret, as a write-only value and store only the credential reference/configuration metadata in normal application storage. Business-profile-specific Git credential overrides are planned after the global flow is validated. Future business-specific settings should store only repository metadata and credential references in the database; raw tokens or private keys should remain in the deployment's configured secret store, such as a local secret mechanism for local mode or AWS Secrets Manager/ECS task secrets for AWS mode.
