...
def resolve_config(repo_root: Path | None = None, cwd: Path | None = None) -> AppConfig:
    ...
    output_dir = _resolve_path(output_dir_value, working_directory) if output_dir_value else working_directory

    database_value = values.get("DATABASE_PATH")
    if database_value:
        database_path = _resolve_path(database_value, working_directory)
    else:
