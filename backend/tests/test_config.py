...
def test_resolve_config_reads_run_mode(tmp_path: Path) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir(parents=True, exist_ok=True)
    config_path = repo_root / ".config"
    config_path.write_text(
...
def test_resolve_config_rejects_invalid_run_mode(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert str(error) == "RUN_MODE must be 'local' or 'aws'"
    else:
        raise AssertionError("Expected invalid RUN_MODE error")
