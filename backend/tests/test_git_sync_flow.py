...
def _write_save_config(tmp_path: Path, data_repo: Path) -> Path:
    config_path = tmp_path / ".config"
    config_path.write_text(
        "\n".join(
            [
...
def test_campaign_save_uses_business_git_settings(monkeypatch, tmp_path: Path) -> None:
    remote = tmp_path / "business-remote.git"
    source = tmp_path / "source"
    data_repo = tmp_path / "efs-business-repo"
    verify_repo = tmp_path / "verify"
    ...
    shutil.copytree(Path(__file__).resolve().parents[2] / "tests" / "data", source, dirs_exist_ok=True)
    _git(source, "add", ".")
    _git_commit(source, "Seed business YAML data")
    _git(source, "branch", "-M", "main")
    _git(source, "remote", "add", "origin", str(remote))
    _git(source, "push", "origin", "main")
    _git(tmp_path, "clone", "--branch", "main", str(remote), str(data_repo))

    config_path = _write_save_config(tmp_path, data_repo)
    monkeypatch.setenv("GPMPE_CONFIG_FILE", str(config_path))
    monkeypatch.setenv("GPMPE_USE_TEST_PATHS", "true")
    ...
    with TestClient(create_app()) as client:
        ...
        git_settings = client.put(
            f"/admin/businesses/{acme['id']}/git-settings",
            json={
                "repo_path": str(data_repo),
                "remote_url": str(remote),
                "remote_name": "origin",
                "branch": "main",
                "user_name": "Business Git Service",
                "user_email": "business-git@example.com",
                "push_enabled": True,
                "credential_provider": "local",
            },
        )
        assert git_settings.status_code == 200, git_settings.text

        campaigns = client.get(f"/businesses/{acme['id']}/campaigns")
        assert campaigns.status_code == 200, campaigns.text
        mothersday = next(row for row in campaigns.json()["items"] if row["campaign_name"] == "mothersday")

        outbound_title = "Saved With Business Git Settings"
        edit = client.patch(
            f"/businesses/{acme['id']}/campaigns/{mothersday['id']}",
            json={"title": outbound_title},
        )
        ...
        save = client.post(
            f"/campaigns/{mothersday['id']}/save",
            json={"commit_message": "Save with business git settings"},
        )
        assert save.status_code == 200, save.text
